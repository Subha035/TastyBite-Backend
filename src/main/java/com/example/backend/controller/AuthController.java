...
public class AuthController {
    ...
    public ResponseEntity<?> googleLogin(@RequestBody Map<String, String> request) {
        try {
            ...
            Map<String, Object> response = authService.loginWithGoogle(idToken, email, name, picture);
            return ResponseEntity.ok(response);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("message", e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("message", "Google login failed: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> me(@RequestHeader(value = "Authorization", required = false) String authHeader) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("message", "Failed to fetch user profile"));
        }
    }
}
